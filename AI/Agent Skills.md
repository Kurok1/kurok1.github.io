...
**维度静态提示词 (Custom Instructions)按需注入的 Skill存在感**每一轮对话都存在，占用 Token。只有触发时才出现，节省资源。**专注度**AI 容易被长指令分散注意力。AI 只在需要时获得“专业技能补丁”。**冲突处理**容易和用户当下的需求打架。系统通常会有一个“调度逻辑”来协调优先级。
## 4. 为什么要这么设计？
