...
```json
{
   "chart_type": "table_2d | line | pie | gauge",
   "title": "一句话图表标题（中文）",
   "intent_summary": "用一句话概括用户想看什么",
   "time": {
      "time_field": "若用户隐含时间则推断常见时间字段语义，否则为 null",
      "time_range": "例如：last_30_days / this_month / 2025-01-01~2025-01-31 / null",
      "time_grain": "day | week | month | quarter | year | null"
   },
   "metrics": [
      {
         "name": "指标语义名，如：订单数/销售额/活跃用户",
         "aggregation": "sum | count | count_distinct | avg | max | min | ratio | null",
         "expression_hint": "如需特殊口径，用自然语言描述，例如：支付金额=实付金额；转化率=下单用户/访问用户",
         "unit": "元/人/次/%/null"
      }
   ],
   "dimensions": [
      {
         "name": "维度语义名，如：渠道/地区/品类/产品",
         "role": "x | series | category | row | null"
      }
   ],
   "filters": [
      {
         "field_hint": "字段语义名，如：地区/渠道/状态",
         "op": "= | in | between | like | > | < | null",
         "value_hint": "用自然语言给出过滤值，如：华东、已支付、北京",
         "confidence": 0
      }
   ],
   "chart_spec": {
      "type": "line | pie | gauge | table_2d"
   },
   "assumptions": [
      "若用户没说清时间范围/指标口径/TopN 等，在此列出你做的最小必要假设"
   ],
   "confidence": 0
}
```
...
- `title`：建议的图表标题（中文），一句话描述。
    
- intent_summary：对用户需求的“意图摘要”，一句话说明用户想看什么。
    
- `time`：时间字段的语义提示（例如“下单时间/支付时间/创建时间”等），不要求真实库字段名。
    
- `metrics`：一个数组，允许多个指标（特别是表格需要多指标列）
	- `metrics[i].name`: 指标的语义名（销售额、订单数、DAU、转化率等）。
	- `metrics[i].aggregation`: 聚合方式 `sum|count|count_distinct|avg|max|min|ratio|null`。
	- `metrics[i].expression_hint`: 当指标不是简单聚合时，用自然语言说明口径/公式，例如：

		- “转化率=下单用户/访问用户”
    
		- “GMV=订单金额（含运费）”
	- `metrics[i].unit`: 单位提示（元、人、次、%、null）
    
- `dimensions`：维度（地区、渠道、品类、产品、用户类型等）及其在图表中的角色。
	- `dimensions[i].name`: 维度语义名。
	- `dimensions[i].role`: 该维度在图表里的角色
    
- `filters`：从自然语言抽取的过滤条件（地区=华东、状态=已支付、渠道 in …）。
	- `field_hint`: 过滤字段的语义提示。
	- `op`: 操作符提示（=, in, between, like, >, <）。
	- `value_hint`: 过滤值的自然语言提示。
	- `confidence`: 模型对该过滤条件是否可靠的置信度（0~1）。
- `assumptions[]`: 当用户缺信息时，模型做的最小必要假设列表。
- `confidence`: 对整体输出（尤其 chart_type 与关键字段）的置信度 0~1。
    

## 图表规格 `chart_spec`

这是最关键的部分：它把“选了什么图”变成“怎么画”。

### `chart_spec.line`

仅当 `chart_type=line` 时应填关键字段。

#### `x_axis.field` + `x_axis.grain`

**含义**：x 轴字段语义（通常时间）和粒度。  
**为什么**：

- 折线的本质是时间序列或连续序列；没有 x 轴分桶就无法生成可读曲线。
    

####  `y_axis[]`

**含义**：一个或多个 y 轴指标引用（`metric_ref` 指向 `metrics` 数组索引），以及格式化类型。  
**为什么**：

- 用引用而不是复制指标定义，避免重复与不一致。
    
- 允许多个 y 指标（虽然你可能先做单指标折线，但结构上保留扩展性）。
    

#### `series`

**含义**：多条线规划：按哪个维度拆分、预期线条数量、是否 TopN。  
**为什么**：

- 用户常说“按渠道看趋势”，这就需要 series 维度。
    
- `expected_line_count` / `top_n` 是工程关键：线条太多会不可读且 SQL 成本高；你可以据此做 TopN+Others 或默认 Top5。
...
####  `top_n` 与 `others_bucket`

**含义**：展示前 N 个类别，其余合并为“其他”。  
**为什么**：

- 饼图类别过多会严重降低可读性。
    
- 这是典型工程化约束：既保证可视化质量，也控制查询结果规模。
...
- Gauge 的核心就是“一个数”，必须明确它是什么以及怎么聚合。
    

####  `target`

**含义**：目标值（如果用户提到了“目标/达成/完成率”），并标明来源 user/inferred。  
**为什么**：

- 是否存在目标值决定 Gauge 是否需要进度/区间。
    
- 标明来源便于你决定是否可信：用户明确给出 vs 模型推断。
...
- 有些 KPI 天然范围 0~1 或 0~100%；有些是金额不适合设 max。
    
- 允许 `max=null` 代表只展示数值，不做刻度范围约束。
...
- 表格最常见是“按 X 分组列出”，rows 就是 group by 的语义。
    

####  `columns`

**含义**：列定义，可以是 metric 或 dimension。  
**为什么**：

- 让表格能同时容纳：行维度 + 多指标列 +（可选）额外维度列。
    
- 结构明确，便于前端表格渲染与后端 SQL select 列表生成。
...
- “Top/排名”必须有排序，否则结果不符合预期。
    
- 明确排序还能避免默认排序造成的不可解释差异。
